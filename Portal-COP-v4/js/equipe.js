...
function atualizarEquipe(colaboradores) {
  ...
  function render(lista) {
    ...
    lista.forEach(colab => {
      ...
      const initials = colab.Nome.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
      const colors = ['#4f8cf7', '#6c5ce7', '#00b894', '#fdcb6e', '#e17055', '#0984e3', '#fd79a8', '#00cec9'];
      const colorIndex = (colab.Nome.length + (colab.Horário || '').length) % colors.length;
      ...
      card.innerHTML = `
        ...
        <div class="team-info">
          <div class="team-name">${colab.Nome}</div>
          <div class="team-horario"><i class="fas fa-clock"></i> ${colab.Horário || 'Não informado'}</div>
          <span class="team-role">${colab.Cargo || 'Colaborador'}</span>
        </div>
      `;
